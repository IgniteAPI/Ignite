...
namespace IgniteAPI.Utils
{
    public static class EnviromentVarLoader
    {
        public static void Load(object target)
        {
            ...
            {
                // 1?? Check if property has the EnvVarAttribute
                ...
                if (attr != null)
                {
                    ...
                    if (value != null && prop.CanWrite)
                    {
                        ...
                        {
                            Console.WriteLine($"?? Failed to set {prop.Name} from {attr.Name}: {ex.Message}");
                        }
                    }
                    continue;
                }

                // 2?? If property is a class (not primitive/string), recurse
